...
const ForRent = () => {
    const queryKeyMovies = 'for-rent-movies-trailers';
    const queryKeyTrailers = 'for-rent-trailers'
                    ...
                    ?.map(movie => {
                        ...
                        if (videoInfo?.results?.length !== 0) {
                            return <Card className="trailers-card" key={movie.id} onClick={handleShow}>
                                <Card.Body>
                                    <Link onClick={handleShow} to={`for-rent-trailers/:${movie.id}`} state={{ trailer, showTrailer }}>
